...
class EventTracker{
    ...
    /**
     * @name addEvent
     * @description This method allows storing an event in the database
     * @param {object} params
    ...
    async addManyEvents(ids, type) {
        try {
            ...
            for(const id of ids) {
                const [savedTime, error] = await Helpers.promiseWrapper(
    ...
    /**
     * Retrieves the type of the last event associated with a given ID.
     * 
     * @name getLastEventTypeById 
     * @param {string} id - The ID for which to retrieve the last event type.
     * @returns {Promise<string>} A promise that resolves to the event type of the last event, or an empty string if no event is found.
    ...
    async isEventAlreadyStarted (id) {
        try {
            await Validations.idValidation(id);
    ...
    async stopEventsInBackground () {
        try {
            ...
            for(const id of ids) {
                const [saved, saveError] = await Helpers.promiseWrapper(
                    this.addEvent({
                        id,
                        type: 'pause'
                    })
     ...
     *              state to the background, it will pause ongoing events by calling `stopEventsInBackground`. It then updates the 
    ...
    async handleAppStateChange(nextAppState) {
        try {
            if (this.appCurrentState.match(/active|foreground/) && nextAppState === 'background') {
              ...
              await this.stopEventsInBackground();
            }
     ...
     * 
     * @name startListening
     * @description Adds an event listener to monitor changes in the application state and calls `handleAppStateChange` 
     ...
     * 
     * @name stopListening
     * @description Removes the event listener that monitors changes in the application state and was calling `handleAppStateChange`.
     * @returns {void}
     */
