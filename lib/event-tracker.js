...
class EventTracker {
	 ...
	 * @param {Array<{type: string, time: string}>} events tracked events, in chronological order
	 * @param {boolean} format if true, returns the time in days-hours-minutes-seconds format, otherwise returns only the milliseconds
	...
	getNetTrackingTime({events = [], format = false}) {
		...
		const openerTypes = ['start', 'resume'];
		const closerTypes = ['pause', 'finish'];

		let openedAt = null;
		const netTime = events.reduce((total, event) => {
			if (openerTypes.includes(event?.type) && !openedAt) {
				openedAt = event.time;
				return total;
			}

			if (closerTypes.includes(event?.type) && openedAt) {
