...
import {OPENER_TYPES, CLOSER_TYPES} from '../utils/eventTypes';
...
class EventTracker {
	...
	async addEvent(params = {}) {
		try {
			const {id, time, payload} = params;

			// Se normaliza una vez y se usa el mismo valor para validar y para guardar:
			// persistir el type crudo dejaba registros que ningún cálculo reconocía
			const type = Validations.normalizeEventType(params.type);
	...
	async getEventsById(id) {
		try {
			...
			// Se devuelve orden cronológico para que ningún consumer tenga que re-ordenar
			return Helpers.sortValidEventsByTime(events).map((event) => Event.parseEventFromDB(event));
		} catch (error) {
			return Promise.reject(error);
		}
	}
	...
	async getLastEventById(id) {
		try {
			...
			// El "último evento" es el cronológico, no el de la tabla: de acá sale el
			// previousType que valida la secuencia, así que una lectura desordenada
			// bloqueaba la reapertura de un ciclo cerrado
			const sortedEvents = Helpers.sortValidEventsByTime(events);
	...
	getElapsedTime({startTime, finishTime, format = true}) {
		...
		const lastTime = finishTime || new Date().toISOString();
	 ...
	 * events with an invalid time are ignored. A clock regression that leaves two
	 * cycles OVERLAPPING in time degrades to an under-count — never an over-count:
	 * sorting interleaves them as `start > start > finish > finish`, and the second
	 * opener is dropped because a span is already open. A regression that does not
	 * overlap the previous cycle is fully absorbed by the sort.
	 *
	...
	getNetTrackingTime({events = [], format = false}) {
		if (!events?.length || !Helpers.isArray(events))
			return format ? Helpers.convertMillisecondsToTime(0) : 0;
	 ...
	 * @param {string} rawType - The type used to filter events.
	...
	async getIdTimeByType(id, rawType) {
		try {
			await Validations.idValidation(id);

			// El filtro de Realm compara el type de forma case-sensitive: sin normalizar,
			// un 'START' válido para la validación no matcheaba ningún registro
			const type = Validations.normalizeEventType(rawType);
			...
			// El "último" evento del tipo es el cronológico, no el de la tabla: un registro
			// multi-ciclo tiene N eventos del mismo type por id
			const sortedEvents = Helpers.sortValidEventsByTime(filteredEvents);
			const lastEvent = sortedEvents[sortedEvents.length - 1];

			if (!lastEvent) return null;

			return Event.parseEventFromDB(lastEvent).time;
		} catch (error) {
			return Promise.reject(error);
		}
	}
...
export {EVENT_TYPES} from '../utils/eventTypes';
